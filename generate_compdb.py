...
def parse_command(command, file):
    ...
    if len(command) >= 3 and command[0] == '/bin/bash':
        command = shlex.split(command[2])
    
    start_index = 0
    for index, split_command in enumerate(command):
        if split_command.endswith('/clang') or split_command.endswith('/clang++'):
            start_index = index
            break
...
with open(args.ninja_file) as ninja_file:
    for line in ninja_file:
        ...
        # NEED EDIT HERE
        if not file.startswith('vendor/qcom/proprietary/chi-cdk'):
            continue
